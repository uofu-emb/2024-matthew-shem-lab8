...
#define TALK_TASK_PRIORITY_LOW      ( tskIDLE_PRIORITY + 2UL )
#define TALK_TASK_PRIORITY_HIGH     ( tskIDLE_PRIORITY + 3UL )
#define BLINK_TASK_PRIORITY     ( tskIDLE_PRIORITY + 4UL )
#define RECEIVE_TASK_PRIORITY   ( tskIDLE_PRIORITY + 1UL )
#define TALK_TASK_STACK_SIZE configMINIMAL_STACK_SIZE
#define BLINK_TASK_STACK_SIZE configMINIMAL_send_taskSTACK_SIZE
#define RECEIVE_TASK_STACK_SIZE configMINIMAL_STACK_SIZE


static struct can2040 cbus;
int on = 0;
int talk_frequency = 3000;
...
void send_task(__unused void *params) {
    while (1) {
        ...
        vTaskDelay(talk_frequency*portTICK_PERIOD_MS);
...
int main( void )
{
    ...
    TaskHandle_t task;
    xTaskCreate(send_task, "TalkThread",
                TALK_TASK_STACK_SIZE, NULL, TALK_TASK_PRIORITY_LOW, &task);

    xTaskCreate(receive_task, "ListenThread",
